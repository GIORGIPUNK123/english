...
export const useOpenGroupLessons = ({
  enabled = true,
  refetchWhenKey,
}: {
  enabled?: boolean;
  ...
  useEffect(() => {
    ...
    const fetchOpenGroupLessons = async () => {
      setLoading(true);
      setError(null);

      try {
        const listOpenGroupLessons = httpsCallable<
          Record<string, never>,
          { lessons: DiscoverableLessonDto[] }
        >(functions, 'listOpenGroupLessons');

        const result = await listOpenGroupLessons({});
        if (cancelled) return;
        ...
        if (cancelled) return;
        setLessons([]);
        setError('Failed to load open group classes.');
      } finally {
        if (!cancelled) setLoading(false);
      }
    };

    fetchOpenGroupLessons();
